# Purpose: GO enrichment on putative core stress genes (from set operations)
# Author: Anna Pardo
...
ipr <- read.delim("//wsl$/Ubuntu/home/leviathan22/core-stress-transcriptome/data/Zm-B73-REFERENCE-NAM-5.0_Zm00001eb.1.interproscan.tsv",
                  header = FALSE)
# annotation from UniProt
upt <- read.delim("//wsl$/Ubuntu/home/leviathan22/core-stress-transcriptome/data/Zm-B73-REFERENCE-NAM.UniProt.GO.tab",
                  header = FALSE)
...
##### 3. Set up GO annotation #####
# view head of ipr
head(ipr)
# the only columns needed are V1 and V14. Drop all other columns
iprgo <- ipr %>% select(V1,V14)
head(iprgo)

# strip transcript identifiers from gene IDs in iprgo
iprgo$V1 <- sapply(strsplit(iprgo$V1,split="_",fixed=TRUE), function(x)(x[1]))
head(iprgo)
# drop duplicate rows
iprgo <- distinct(iprgo)
# drop rows containing only a space
iprgo <- iprgo %>% filter(V14!="")
# rearrange the data such that there is 1 row per gene ID and all GO terms are together in the V14 column
# write a function to help with this
combine_lists <- function(geneid,df){
  df1 <- df %>% filter(V1 == geneid)
  vec <- c()
  for(i in 1:nrow(df1)){
    vec[i] <- c(unlist(strsplit(df1[i,2],"\\|")))
  }
  return(vec)
}

# data structure that needs to be generated: named list where names are gene IDs and values are
## vectors of GO terms
zmGO <- list()
namelist <- unique(iprgo$V1)
for(i in 1:length(namelist)){
  df <- iprgo %>% filter(V1 == namelist[i])
  vec <- c()
  for(j in 1:nrow(df)){
    gostring <- df[j,2]
    golist <- strsplit(gostring,split = "\\|")
    for(k in golist[[1]]){
      vec <- append(vec,k)
    }
  }
...
# add any GO terms from upt (UniProt annotation) that are not already present
for(i in 1:length(unique(upt$V1))){
  geneid <- unique(upt$V1)[i]
  # generate vec as above
  df <- upt %>% filter(V1 == geneid)
  vec <- c()
  for(j in 1:nrow(df)){
    gostring <- df[j,3]
    golist <- strsplit(gostring,split = "\\|")
  ...
  # iterate through vec and add GO terms to vector within zmGO (appropriate gene) if not present
  for(l in vec){
    if(!(l %in% zmGO[[geneid]])){
      zmGO[[geneid]] <- append(zmGO[[geneid]],l)
    }
  }
}

# save zmGO as a json file
jsondata <- toJSON(zmGO,indent = 4)
write(jsondata,"//wsl$/Ubuntu/home/leviathan22/core-stress-transcriptome/data/Zmays_B73V5_GO_annotation.json")
...
write.table(corego, file = "//wsl$/Ubuntu/home/leviathan22/core-stress-transcriptome/data/GO_enrichment_coregenes_all.txt",
            sep = "\t",row.names = FALSE,col.names = TRUE)

##### 7. GO enrichment data exploration #####
# how many unique GO terms are enriched in upregulated genes?
upgo <- corego %>% filter(regset=="Upregulated")
length(unique(upgo$GO.term))

downgo <- corego %>% filter(regset=="Downregulated")
length(unique(downgo$GO.term))
